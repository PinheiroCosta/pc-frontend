...
const Tools = () => {
  ...
  const [categoryFilter, setCategoryFilter] = useState<Tool["category"] | null>(
    null,
  );
  const pageSize = 6;
  ...
  if (tools.length === 0) {
    return (
      <ErrorMessage
        title="Error"
        message="Nenhuma Ferramenta foi encontrada."
      ></ErrorMessage>
    );
  }
  ...
  return (
    <Container className="mt-5 ">
      ...
      {categoryFilter && (
        <InfoMessage
          title=""
          message={`Filtrando por categoria: ${categoryFilter}`}
        >
          <button
